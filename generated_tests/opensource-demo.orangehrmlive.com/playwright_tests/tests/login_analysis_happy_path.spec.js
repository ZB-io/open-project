...
test('login_analysis_happy_path', async ({ page, context }) => {
  try {
    ...
    //   3. page.locator('input.oxd-input') >> nth=0 (confidence: 78%, strategy: css_stable_class, unique: false)
    ...
    //   5. page.locator('input.oxd-input') >> nth=1 (confidence: 78%, strategy: css_stable_class, unique: false)
    //   6. page.locator('xpath=html/body/div[1]/div[1]/div/div[1]/div/div[2]/div[2]/form/div[2]/div/div[2]/input') (confidence: 50%, strategy: xpath, unique: true)
    ...
    //   3. page.getByText('Login') >> nth=1 (confidence: 88%, strategy: text, unique: false)
    //   4. page.locator('#app').getByRole('button', { name: 'Login' }) (confidence: 82%, strategy: parent_id_role, unique: true)
    //   5. page.locator('button.oxd-button.orangehrm-login-button') (confidence: 80%, strategy: css_combined_classes, unique: true)
    //   6. page.locator('button.oxd-button') (confidence: 78%, strategy: css_stable_class, unique: true)
    //   7. page.locator('xpath=html/body/div[1]/div[1]/div/div[1]/div/div[2]/div[2]/form/div[3]/button') (confidence: 50%, strategy: xpath, unique: true)
    await page.locator('form[action*="validate"]').getByRole('button', { name: 'Login' }).click();

    // Step 5: Verify successful login by confirming navigation to the dashboard page.
