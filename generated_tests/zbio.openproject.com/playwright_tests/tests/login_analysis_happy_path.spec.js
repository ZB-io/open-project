...
test('login_analysis_happy_path', async ({ page, context }) => {
  try {
    ...
    // Step 4: Click the 'Sign in' button to submit the credentials.
    // Captured selectors:
    ...
    // Step 5: Verify that the login was successful and the user is redirected to the application's home page.
    // Using a regex to handle optional trailing slashes in the URL.
    const expectedUrlRegex = new RegExp(`^${process.env.BASE_HOST_URL || process.env.LOGIN_HOST_URL}/?$`);
