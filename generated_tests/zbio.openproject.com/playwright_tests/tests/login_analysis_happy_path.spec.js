...
test('login_analysis_happy_path', async ({ page, context }) => {
  try {
    ...
    //   4. page.locator('input[name="username"]') (confidence: 87%, strategy: name_attribute, unique: false)
    ...
    //   4. page.locator('input[name="password"]') (confidence: 87%, strategy: name_attribute, unique: false)
    ...
    //   3. page.locator('input[value="Sign in"]') (confidence: 89%, strategy: css_tag_value, unique: false)
    //   4. page.locator('input[data-controller="disable-when-clicked"]') (confidence: 85%, strategy: data_attr_controller, unique: true)
    //   5. page.locator('#login-form').getByRole('button', { name: 'Sign in' }) (confidence: 82%, strategy: parent_id_role, unique: true)
    await page.locator('form[action*="login"]').getByRole('button', { name: /Sign in/ }).click();

    // Step 5: Verify successful login by checking for navigation to the homepage.
    // Using a regex to handle potential redirects to different post-login pages like /projects or /my/page
    await page.waitForURL(new RegExp(`${process.env.LOGIN_HOST_URL || process.env.BASE_HOST_URL}/.*`), { timeout: 60000 });
    await expect(page).toHaveURL(new RegExp(`${process.env.LOGIN_HOST_URL || process.env.BASE_HOST_URL}/`));
