...
test('login_analysis_happy_path', async ({ page, context }) => {
  try {
    ...
    //   5. page.locator('input[type="submit"]') (confidence: 88%, strategy: input_type, unique: false)
    await page.locator('#login-form').getByRole('button', { name: 'Sign in' }).click();

    // Step 5: Verify successful login by checking the page URL has changed to the homepage.
    const expectedUrl = `${process.env.LOGIN_HOST_URL || process.env.BASE_HOST_URL}/`;
    // Use a regular expression to handle optional trailing slashes or redirects to a sub-path like /my/page
    await page.waitForURL(new RegExp(`^${(process.env.LOGIN_HOST_URL || process.env.BASE_HOST_URL)}`));
