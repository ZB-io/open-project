...
test('login_analysis_happy_path', async ({ page, context }) => {
  try {
    ...
    // Step 5: Verify successful login by checking for redirection to the admin subscriptions page.
    const expectedUrl = `${process.env.LOGIN_HOST_URL || process.env.BASE_HOST_URL}/admin/subscriptions/expired`;
    await page.waitForURL(new RegExp('.*/admin/subscriptions/expired'));
